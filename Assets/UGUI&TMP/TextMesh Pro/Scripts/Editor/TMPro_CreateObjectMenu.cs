...
namespace TMPro.EditorUtilities
{
    public static class TMPro_CreateObjectMenu
    {
        ...
        static void CreateTextMeshProObjectPerform(MenuCommand command)
        {
            ...
            {
                if (!assembly.ToString().Contains("Assembly-CSharp")) continue;
                foreach (var t in assembly.GetTypes())
                {
                    if (!t.ToString().Equals("UIKit.LocalisationText")) continue;
                    go.AddComponent(t);
                    break;
                }
            }
        ...
        [MenuItem("GameObject/UI/Toggle - TextMeshPro", false, 2036)]
        public static void AddToggle(MenuCommand menuCommand)
        {
            GameObject go = TMP_DefaultControls.CreateToggle(GetStandardResources());
            PlaceUIElementRoot(go, menuCommand);
            SetPosition(go);
        }
