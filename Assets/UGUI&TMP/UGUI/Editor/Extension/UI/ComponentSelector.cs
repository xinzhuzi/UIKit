...
public class ComponentSelector : ScriptableWizard
{
   ...
   static public void ShowUIEffect (OnSelectionCallback cb)
    {
        string[] extensions = new string[] { ".prefab" };
        ComponentSelector comp = ScriptableWizard.DisplayWizard<ComponentSelector>("Effect UI");
        comp.mTitle = "Effect UI";
        comp.mType = typeof(GameObject);
        comp.mCallback = cb;
        comp.mExtensions = extensions;
        Object[] objs = Resources.FindObjectsOfTypeAll(typeof(GameObject));
        for(int i = 0; i < objs.Length; ++i)
        {
            if (!objs[i].name.Contains("uief_") && !objs[i].name.Contains("ef_ui"))
            {
                objs[i] = null;
            }
        }
        ...
        else
        {
            for (int i = 0; i < comp.mObjects.Length; ++i)
            {
                if (comp.mObjects[i] != null )
                {
                    Object obj = comp.mObjects[i];
                    string path = AssetDatabase.GetAssetPath(obj);
                    if (string.IsNullOrEmpty(path) || !path.Contains("effect/UI"))
                    {
	...
	void Search ()
	{
		...
		if (mExtensions != null)
		{
            ...
            if (mType == typeof(SpriteAtlas))
            {
                paths = new List<string>();
                string[] guids = AssetDatabase.FindAssets("t:Prefab", new string[] { "Assets/CC_Atlases", "Assets/Res_Best/Prefabs/Atlas" });
            ...
            {
                paths = new List<string>(AssetDatabase.GetAllAssetPaths());
            }
			...
			for (int i = 0; i < paths.Count; ++i)
			{
				...
				EditorUtility.DisplayProgressBar("Loading", "Searching assets, please wait...", (float)i / paths.Count);
